...
def plot_setup(xlabel='Time', ylabel='Value', title=''):
    plt.grid()
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.title(title)
...
def get_smoothed(data, window_size=3, min_periods=None):
    return np.array(pd.Series(data).rolling(window=window_size, center=True, min_periods=min_periods).mean())

def derivative(array, interval):
    array = np.asanyarray(array)
    return (array[2:] - array[:-2]) / (2*interval)
